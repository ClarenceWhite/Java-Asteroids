...
public class Window extends Application{
    ...
    public void start(Stage stage){
        ...
        //This loop is used to generate different numbers of asteroids, could be written in a class and match with gaming levels
        for (int i = 0; i < 5; i++) {
        ...
        //for each asteroid in asteroids list
...
//                    System.out.println("updated:"+lastPressProcessed);
                ...
                for (Bullet bullet:bullets) {
                    ...
                    if (bullet.shouldRemove()){
                        bullets.remove(bullet); //remove bullet from the bullets list
                    ...
                    List<Asteroids> collisions = asteroids.stream()
                            .filter(asteroid -> asteroid.collide(bullet)).toList(); //collect those asteroids to list

                    if(collisions.size() != 0) { //if the collision list is not empty
                    ...
                    collisions.forEach(collided -> { //for each asteroid which has collided with a bullet
                        ...
                        asteroids.remove(collided); //remove collided asteroids from asteroids list
